      ...
      z.nested {
        ...
        if instance_variables.empty?
          ...
          instance_variables.each { |v|
            z.nl
            v.to_s[1..-1].to_zamls(z)       # Remove leading '@'
            z.emit(': ')
            yaml_property_munge(instance_variable_get(v)).to_zamls(z)
          }
        end
      }
    }
    ...
    z.with_structured_prefix(self) do
      ...
      when (self =~ /\n/ and self !~ /\A\s/ and self !~ /\s\z/)
        ...
        escaped = gsub(/[\\"\x00-\x1F]/) do |c|
          ZAMLS_ESCAPES[c] || "\\x#{c[0].ord.to_s(16)}"
        end
        z.emit("\"#{escaped}\"")
      end
    end
    ...
    z.first_time_only(self) {
      z.nested {
        if empty?
          ...
          sort_by(&:first).each { |k, v|
            z.nl
            z.prefix_structured_keys('? ') { k.to_zamls(z) }
            z.emit(': ')
            v.to_zamls(z)
          }
        end
