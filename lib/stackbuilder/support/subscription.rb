...
class Subscription
  ...
  class WaitResponse
    ...
    def all
      ...
      passed.each { |vm| result[vm] = "success" }
      failed.each { |vm| result[vm] = "failed" }
      unaccounted_for.each { |vm| result[vm] = "unaccounted_for" }
  ...
  def wait_for_hosts(topic, hosts, timeout = 1)
    ...
    while !all_accounted_for?(return_results, hosts) && !timed_out(start_time, timeout)
      begin
        ...
        puts "Timed out waiting for message #{e} .. retrying"
      end
    end
