...
describe_stack 'nameserver' do
  ...
  host("e1-lb-001.mgmt.space.net.local") do |host|
    host.to_enc['role::loadbalancer']['virtual_servers']['e1-ns-vip.space.net.local']['healthchecks'].should include(
      {'healthcheck'=>'COMMAND_CHECK','command'=>'/usr/bin/host -4 -W 3 -t A -s apt.mgmt.space.net.local'},
      {'healthcheck'=>'COMMAND_CHECK','command'=>'/usr/bin/host -4 -W 3 -t A -s gw-vip.front.space.net.local'},
      {'healthcheck'=>'COMMAND_CHECK','command'=>'/usr/bin/host -4 -W 3 -t A -s gw-vip.space.net.local'}
    )
  end
end
