...
describe 'bind servers' do
  ...
  describe_stack 'load balancer config for bind services should be correct' do
    given do
      stack 'lb' do
        loadbalancer_service
      end
    ...
    host('oy-lb-001.mgmt.oy.net.local') do |host|
      expect(host.to_enc['role::loadbalancer']['virtual_servers']['oy-ns-vip.mgmt.oy.net.local']['type']).to eq 'bind'
      expect(host.to_enc['role::loadbalancer']['virtual_servers']['oy-ns-vip.mgmt.oy.net.local']['ports']).to eq [53]
      healthchecks = host.to_enc['role::loadbalancer']['virtual_servers']['oy-ns-vip.mgmt.oy.net.local']['healthchecks']
      expect(healthchecks).to eq(
        [{ "healthcheck" => "MISC_CHECK",
           "arg_style" => "PARTICIPATION",
           "path" => "/opt/youdevise/keepalived/healthchecks/bin/check_participation.rb",
           "url_path" => "/participation" },
