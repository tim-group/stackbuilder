...
describe_stack 'creates the correct enc on backup machines depending on whether upload_backups is specified or not' do
  given do
    stack "mysql" do
      mysql_cluster "mydb" do
        self.database_name = 'test'
        ...
        self.primary_site_backup_instances = 1
        each_machine do |machine|
          machine.upload_backups if machine.mgmt_fqdn == 'production-mydbbackup-001.mgmt.earth.net.local'
        end
      end
    end
    env "production", :production => true, :primary_site => "space", :secondary_site => "earth" do
  ...
  host("production-mydbbackup-001.mgmt.space.net.local") do |host|
    enc = host.to_enc
    expect(enc['s3_backup']).to be_nil
  end

  host("production-mydbbackup-001.mgmt.earth.net.local") do |host|
    enc = host.to_enc
    expect(enc['s3_backup']).not_to be_nil
  end
end

describe_stack 'creates the correct enc on backup machines depending on whether db_refresher is specified or not' do
  given do
    stack "mysql" do
      mysql_cluster "mydb" do
        self.database_name = 'test'
        ...
        self.primary_site_backup_instances = 1
        each_machine do |machine|
          machine.db_refresher if machine.mgmt_fqdn == 'production-mydbbackup-001.mgmt.earth.net.local'
        end
      end
    end
    env "production", :production => true, :primary_site => "space", :secondary_site => "earth" do
  ...
  host("production-mydbbackup-001.mgmt.space.net.local") do |host|
    enc = host.to_enc
    expect(enc['dbrefresh']).to be_nil
  end

  host("production-mydbbackup-001.mgmt.earth.net.local") do |host|
    enc = host.to_enc
    expect(enc['dbrefresh']).not_to be_nil
  end
end

describe_stack 'creates the correct enc on backup machines depending on whether snapshot_capable is specified or not' do
  given do
    stack "mysql" do
      mysql_cluster "mydb" do
        self.database_name = 'test'
        ...
        self.primary_site_backup_instances = 1
        each_machine do |machine|
          machine.snapshot_capable if machine.mgmt_fqdn == 'production-mydbbackup-001.mgmt.earth.net.local'
        end
      end
    end
    env "production", :production => true, :primary_site => "space", :secondary_site => "earth" do
  ...
  host("production-mydbbackup-001.mgmt.space.net.local") do |host|
    enc = host.to_enc
    expect(enc['db_snapshot']).to be_nil
  end

  host("production-mydbbackup-001.mgmt.earth.net.local") do |host|
    enc = host.to_enc
    expect(enc['db_snapshot']).not_to be_nil
  end
end
