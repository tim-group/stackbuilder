...
describe 'kubernetes' do
  ...
  let(:dns_resolver) do
    MyTestDnsResolver.new('e1-x-vip.space.net.local' => '3.1.4.1',
                          ...
                          'space-kvm-001.space.net.local' => '4.5.6.7',
                          'space-kvm-002.space.net.local' => '4.5.6.8')
  end
  let(:hiera_provider) do
    TestHieraProvider.new(
      'stacks/application_credentials_selector' => 0,
      'kubernetes/hosts/space' => [
        'space-kvm-001.space.net.local',
        'space-kvm-002.space.net.local'
      ])
  end
...
EOL
  ...
  describe 'dependencies' do
    ...
    it 'should create the correct kube proxy ingress network policies for a service' do
      factory = eval_stacks do
      ...
      machine_sets = factory.inventory.find_environment('e1').definitions['test_app_server'].k8s_machinesets
      ...
      expect(network_policies.first['metadata']['name']).to eql('allow-kube-proxy-in-to-app1-8000')
      expect(network_policies.first['metadata']['namespace']).to eql('e1')
      expect(network_policies.first['metadata']['labels']).to eql(
        'stack' => 'test_app_server',
        'machineset' => 'app1',
        'app.kubernetes.io/name' => 'app1',
        'app.kubernetes.io/instance' => 'e1-test_app_server-app1',
        'app.kubernetes.io/component' => 'app_service',
        'app.kubernetes.io/version' => '1.2.3',
        'app.kubernetes.io/managed-by' => 'stacks'
      )
      ...
      expect(network_policies.first['spec']['ingress'].first['from']).to include('ipBlock' => { 'cidr' => '4.5.6.7/32' })
      expect(network_policies.first['spec']['ingress'].first['from']).to include('ipBlock' => { 'cidr' => '4.5.6.8/32' })
      expect(network_policies.first['spec']['ingress'].first['ports'].first['protocol']).to eql('TCP')
      expect(network_policies.first['spec']['ingress'].first['ports'].first['port']).to eq('http')
    end
    ...
    it 'should create the correct ingress network policies for a service in kubernetes when another non kubernetes service depends on it' do
      ...
      expect(network_policies[1]['metadata']['name']).to eql('allow-e1-app1-in-to-app2-8000')
      expect(network_policies[1]['metadata']['namespace']).to eql('e1')
      expect(network_policies[1]['metadata']['labels']).to eql(
      ...
      expect(network_policies[1]['spec']['podSelector']['matchLabels']).to eql(
        'app.kubernetes.io/instance' => 'e1-test_app_servers-app2'
      )
      expect(network_policies[1]['spec']['policyTypes']).to eql(['Ingress'])
      expect(network_policies[1]['spec']['ingress'].size).to eq(1)
      expect(network_policies[1]['spec']['ingress'].first['from'].size).to eq(2)
      expect(network_policies[1]['spec']['ingress'].first['ports'].size).to eq(1)
      expect(network_policies[1]['spec']['ingress'].first['from']).to include('ipBlock' => { 'cidr' => '3.1.4.1/32' })
      expect(network_policies[1]['spec']['ingress'].first['from']).to include('ipBlock' => { 'cidr' => '3.1.4.2/32' })
      expect(network_policies[1]['spec']['ingress'].first['ports'].first['protocol']).to eql('TCP')
      expect(network_policies[1]['spec']['ingress'].first['ports'].first['port']).to eq(8000)
    end
    ...
    it 'should create the correct egress network policies for a service in kubernetes when that service depends on another non kubernetes service' do
      ...
      expect(network_policies.size).to eq(2)
      expect(network_policies[1]['metadata']['name']).to eql('allow-app2-out-to-e1-app1-8000')
      expect(network_policies[1]['metadata']['namespace']).to eql('e1')
      expect(network_policies[1]['metadata']['labels']).to eql(
        'stack' => 'test_app_servers',
        'machineset' => 'app2',
      ...
      expect(network_policies[1]['spec']['policyTypes']).to eql(['Egress'])
      expect(network_policies[1]['spec']['egress'].size).to eq(1)
      expect(network_policies[1]['spec']['egress'].first['to'].size).to eq(1)
      expect(network_policies[1]['spec']['egress'].first['ports'].size).to eq(1)
      expect(network_policies[1]['spec']['egress'].first['to']).to include('ipBlock' => { 'cidr' => '3.1.4.4/32' })
      expect(network_policies[1]['spec']['egress'].first['ports'].first['protocol']).to eql('TCP')
      expect(network_policies[1]['spec']['egress'].first['ports'].first['port']).to eq(8000)
    end
    ...
    it 'should create the correct network policies for two services in kubernetes in the same environment when one service depends on the other' do
      ...
      ingress = app1_network_policies[1]['spec']['ingress']
      expect(app1_network_policies.size).to eq(2)
      expect(app1_network_policies[1]['metadata']['name']).to eql('allow-e1-app2-in-to-app1-8000')
      expect(app1_network_policies[1]['metadata']['namespace']).to eql('e1')
      expect(app1_network_policies[1]['metadata']['labels']).to eql(
        'stack' => 'test_app_servers',
        'machineset' => 'app1',
        'app.kubernetes.io/name' => 'app1',
      ...
      egress = app2_network_policies[1]['spec']['egress']
      expect(app2_network_policies.size).to eq(2)
      expect(app2_network_policies[1]['metadata']['name']).to eql('allow-app2-out-to-e1-app1-8000')
      expect(app2_network_policies[1]['metadata']['namespace']).to eql('e1')
      expect(app2_network_policies[1]['metadata']['labels']).to eql(
        'stack' => 'test_app_servers',
        'machineset' => 'app2',
        'app.kubernetes.io/name' => 'app2',
        'app.kubernetes.io/instance' => 'e1-test_app_servers-app2',
      ...
      expect(app2_network_policies[1]['spec']['podSelector']['matchLabels']).to eql(
        'app.kubernetes.io/instance' => 'e1-test_app_servers-app2'
      )
      expect(app2_network_policies[1]['spec']['policyTypes']).to eql(['Egress'])
...
depends on the other' do
      ...
      ingress = app1_network_policies[1]['spec']['ingress']
      expect(app1_network_policies.size).to eq(2)
      expect(app1_network_policies[1]['metadata']['name']).to eql('allow-e2-app2-in-to-app1-8000')
      expect(app1_network_policies[1]['metadata']['namespace']).to eql('e1')
      expect(app1_network_policies[1]['spec']['podSelector']['matchLabels']).to eql(
        'app.kubernetes.io/instance' => 'e1-test_app_server1-app1'
      )
      expect(app1_network_policies[1]['spec']['policyTypes']).to eql(['Ingress'])
      expect(ingress.size).to eq(1)
      expect(ingress.first['from'].size).to eq(1)
      expect(ingress.first['ports'].size).to eq(1)
      expect(ingress.first['from'].first['podSelector']['matchLabels']).to eql(
      ...
      expect(app2_network_policies.size).to eq(2)
      expect(app2_network_policies[1]['metadata']['name']).to eql('allow-app2-out-to-e1-app1-8000')
      expect(app2_network_policies[1]['metadata']['namespace']).to eql('e2')
      expect(app2_network_policies[1]['spec']['podSelector']['matchLabels']).to eql(
        'app.kubernetes.io/instance' => 'e2-test_app_server2-app2'
      )
      expect(app2_network_policies[1]['spec']['policyTypes']).to eql(['Egress'])
